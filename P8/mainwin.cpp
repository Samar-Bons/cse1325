...
: data{Gtk::manage(new Gtk::Label{"Data: "})},
  ...
  shelter{"new_shelter"}
 {
    ...
    Gtk::MenuItem *menuitem_list_clients = Gtk::manage(new Gtk::MenuItem("_List_Clients", true));
    menuitem_list_clients->set_tooltip_markup("List all clients. ");
    menuitem_list_clients->signal_activate().connect([this] {this->on_list_clients_click();});
    clientmenu->append(*menuitem_list_clients); 

    Gtk::MenuItem *menuitem_adopt = Gtk::manage(new Gtk::MenuItem("_Adopt_animal", true));
    menuitem_adopt->set_tooltip_markup("adopt an animal ");
    menuitem_adopt->signal_activate().connect([this] {this->on_adopt_animal_click();});
    clientmenu->append(*menuitem_adopt); 

    Gtk::MenuItem *menuitem_list_adopted = Gtk::manage(new Gtk::MenuItem("list_Adopted_animals", true));
    menuitem_list_adopted->set_tooltip_markup("list adopted animals ");
    menuitem_list_adopted->signal_activate().connect([this] {this->on_list_adopted_click();});
    clientmenu->append(*menuitem_list_adopted); 
...
void Mainwin::on_list_clients_click(){
    ...
    data->set_text(os.str());

}

void Mainwin::on_adopt_animals_click(){
    Gtk::Dialog* dialog = new Gtk::Dialog{"Adopt an animal.", *this};

    auto hbox = dialog->get_content_area();

    Gtk::ComboBoxText* comboboxtext1{Gtk::manage(new Gtk::ComboBoxText{true})};
    Gtk::ComboBoxText* comboboxtext2{Gtk::manage(new Gtk::ComboBoxText{true})};

    hbox->pack_start(*comoboxtext1);
    hbox->pack_start(*comoboxtext2);

    comboboxtext1->set_title("Clients");
    comboboxtext2->set_title("Available animals");

    for(int i = 0; i < shelter.num_clients(); i++){
        comboboxtext1->append(to_string(shelter.client(i)));
    }
    Animal anima;
    for(int j = 0; j < shelter.num_animals(); j++){
        anima = shelter.animal(j);
        comboboxtext2->append(anima.name());
    }
    comboboxtext1->set_active(1);
    comboboxtext2->set_active(1);

    hbox->show_all();
    dialog->run();

    shelter.adopt(shelter.client(comboboxtext1->get_active_row_number()),shelter.animal(comboboxtext2->get_active_row_number()));
}

void Mainwin::on_list_adopted_click(){
    Gtk::Dialog* dialog1 = new Gtk::Dialog{"List adopted animals.", *this};

    auto hbox = dialog1->get_content_area();

    Gtk::ComboBoxText* comboboxtext3{Gtk::manage(new Gtk::ComboBoxText{true})};

    hbox->pack_start(*comoboxtext3);

    comboboxtext3->set_title("Clients");
    

    for(int i = 0; i < shelter.num_clients(); i++){
        comboboxtext3->append(to_string(shelter.client(i)));
    }
    ...
    clinta = shelter.client(comboboxtext3->get_active_row_number());

    Animal anima2;
    std::string listanima = "";

    for (int j = 0; j < clinta.num_adopted(); j++){
        anima2 = clinta.animal(j);
        listanima = listanima + anima2.name()+"\n";
    }
    data->set_text(listanima);
...
std::string to_string(Client& client){
        std::ostringstream os;
        os<<client;
        return os.str();
    }
